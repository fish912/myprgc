...
def span2str(triples, tokens):
    ...
    for triple in triples:
        ...
        output.append((sub, obj, rel, triple[0][-1], triple[1][-1]))
    return output
        ...
        for idx in range(bs):
            if ex_params['ensure_rel']:
                pre_triples = tag_mapping_corres(predict_tags=pred_seqs[xi_index[idx]:xi_index[idx + 1]],
                                                 ...
                                                 label2idx_sub=params.label2idSub,
                                                 label2idx_obj=params.label2idObj)
            else:
...
if __name__ == '__main__':
    ...
    model, optimizer, f1 = utils.load_checkpoint(os.path.join(params.model_dir, args.restore_file + '.pth.tar'))
