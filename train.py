...
def train(model, data_iterator, optimizer, params, ex_params):
    ...
    # one epoch
    t = trange(len(data_iterator), ncols=200, ascii=True)
    for step, _ in enumerate(t):
        # fetch the next training batch
        batch = next(iter(data_iterator))
        batch = tuple(t.to(params.device) for t in batch)
        # seq_tags batch * 2 * max_text_len  头实体和尾实体BI标记
        # relations batch 里面存放关系的id
        # corres_tags batch * max_text_len * max_text_len  sub_head位置和obj_head位置的标记
        # rel_tags batch * rel_num
...
def train_and_evaluate(params, ex_params, restore_file=None):
    ...
    st3 = datetime.now()
    print("get train and val dataloader time: ", st2 - st1, st3 - st2)
    ...
    if restore_file is not None:
        ...
        if os.path.exists(restore_path):
            ...
            # 读取checkpoint
            model_static, model_config, optimizer_static, params.best_val_f1 = utils.load_checkpoint(restore_path)
            model = BertForRE.from_pretrained(config=model_config,
                                              pretrained_model_name_or_path=params.bert_model_dir,
                                              params=params, state_dict=model_static)
            restore = True
    if not restore:
    ...
    # if optimizer_static is not None:
    #     optimizer.load_state_dict(optimizer_static)

    if model.get_input_embeddings().num_embeddings != len(params.tokenizer):
        # 由于分词器添加了新词，要把模型的embedding同步，否则报错
        logging.info("Tokenizer vocab_len update to {}".format(len(params.tokenizer)))
        model.resize_token_embeddings(len(params.tokenizer))
        params.tokenizer.save_pretrained(params.tokenizer_dir)
    model.to(params.device)
    for state in optimizer.state.values():
        for k, v in state.items():
            if isinstance(v, torch.Tensor):
                state[k] = v.to(params.device)
    ...
    for epoch in range(1, args.epoch_num + 1):
        ...
        val_f1 = val_metrics['f1']  # 0~1 越大越好
        improve_f1 = val_f1 - params.best_val_f1

        # Save weights of the network
        # unwrap_model = accelerator.unwrap_model(model)
        unwrap_model = model
        model_to_save = unwrap_model.module if hasattr(unwrap_model,
                                                       'module') else unwrap_model  # Only save the model it-self
        # stop training based params.patience
        if improve_f1 > 0:
            logging.info("- Found new best F1")
            params.best_val_f1 = val_f1
        ...
        logging.info(
            "Epoch duration: {}".format(epoch_end_time - epoch_start_time)
        )
        # Early stopping and logging best f1
        if (patience_counter > params.patience_num and epoch > params.min_epoch_num) or epoch == args.epoch_num:
            logging.info("Best val f1: {:05.2f}".format(params.best_val_f1))
            break
    end_time = datetime.now()
    logging.info("Total training duration: {}".format(datetime.now() - end_time))


if __name__ == '__main__':
    args = parser.parse_args()
    params = utils.Params("org_seq", 'new_custom')
    ex_params = {
    ...
    st = datetime.now()
    train_and_evaluate(params, ex_params, 'best')
    print(f"Total training duration: {datetime.now() - st}")
