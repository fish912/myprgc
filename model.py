...
class BertForRE(BertPreTrainedModel):
    def __init__(self, config, params):
        ...
        self.seq_tag_size_obj = params.seq_tag_size_obj
        ...
        self.sequence_tagging_obj = MultiNonLinearClassifier(config.hidden_size * 2, self.seq_tag_size_obj, params.drop_prob)
    ...
    def forward(
        ...
        """
        # get params for experiments 临界值
        ...
        # ablation study
        ensure_corres, ensure_rel = True, True
        # pre-train model
        outputs = self.bert(
            input_ids,
        ...
        if ensure_corres:
            ...
            # expand 复制
            sub_extend = sequence_output.unsqueeze(2).expand(-1, -1, seq_len, -1)  # (batch_size, seq_len, seq_len, hidden_size)
            obj_extend = sequence_output.unsqueeze(1).expand(-1, seq_len, -1, -1)  # (batch_size, seq_len, seq_len, hidden_size)
            # batch x seq_len x seq_len x 2*hidden
            corres_pred = torch.cat([sub_extend, obj_extend], 3)  # (batch_size, seq_len, seq_len, hidden_size * 2)
            # (bs, seq_len, seq_len)
        ...
        if seq_tags is not None:
            ...
            loss_seq_obj = (loss_func(output_obj.view(-1, self.seq_tag_size_obj),
