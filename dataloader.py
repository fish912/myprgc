...
class CustomDataLoader(object):
    def __init__(self, params):
        ...
        #                                do_lower_case=False)
        # tt2 = AutoTokenizer.from_pretrained('THUDM/chatglm3-6b', trust_remote_code=True)
    ...
    def collate_fn_train(features):
        ...
        corres_tags = torch.tensor(np.array([f.corres_tag for f in features]), dtype=torch.long)
    ...
    def get_features(self, data_sign, ex_params):
        ...
        if os.path.exists(cache_path) and self.data_cache:
            ...
            with open(self.data_dir / f'rel2id.json', 'r', encoding='utf-8') as f_re:
                rel2idx = json.load(f_re)
            # get examples
            if data_sign in ("train", "val", "test", "pseudo", 'EPO', 'SEO', 'SOO', 'Normal', '1', '2', '3', '4', '5'):
                examples = read_examples(self.data_dir, self.tokenizer, data_sign=data_sign, rel2idx=rel2idx)
            else:
